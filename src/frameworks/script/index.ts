...
const createUser = async (user: SignupInput): Promise<ID> => {
    const presenter: SignupPresenter = app.container.resolve<SignupPresenter>('signupPresenter');
    const response = await app.main.signUp(user);
    const output = await presenter.present(response);

    console.log(`User created: (#${output.id}) ${user.firstname}`);
...
(async (): Promise<void> => {
    try {
        console.log("db users collection start",db.users.find())
        ...
        console.log("db users collection end",db.users.find({}))
    } catch (error) {
        console.error(error);
    }
})();
